...
builder.Services.AddTransient<DaoTraining>();
...
try
{
    ...
    // Training
    app.MapGet("/getTraining", async (DaoTraining dao) =>
    {
        return await dao.GetAllAsync();
    });
    app.MapPost("/addTraining", async (Training training, DaoTraining dao) =>
    {
        return await dao.AddAsync(training);
    });

    app.MapPost("/deleteTraining", async (Training training, DaoTraining dao) =>
    {
        return await dao.DeleteAsync(training.Id);
    });

    app.MapPost("/updateTraining", async (Training training, DaoTraining dao) =>
    {
        return await dao.UpdateAsync(training);
    });

    app.MapPost("/findByIdTraining", async (Training training, DaoTraining dao) =>
    {
        return await dao.GetAsync(training.Id);
    });
}

catch (Exception ex)
{
	app.MapGet("/", () => { return ex; });
}
